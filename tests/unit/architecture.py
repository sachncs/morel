...
def documented() -> dict[str, set[str]]:
    """Parse the extension-point table out of docs/concepts/architecture.md.

    Falls back to docs/ARCHITECTURE.md so an old checkout still
    passes; the new location is preferred.
    """
    candidates = [
        SOURCE.parent / "docs" / "concepts" / "architecture.md",
        SOURCE.parent / "docs" / "ARCHITECTURE.md",
    ]
    text = ""
    ...
    if not text:
        raise FileNotFoundError(
            f"docs/concepts/architecture.md not found (tried {[str(c) for c in candidates]})"
        )
