...
def eval(argv: list[str]) -> int:
    ...
    The ``rank`` and ``robustness`` subcommands build a real Pipeline
    from the config (the same synthetic-but-non-random path that
    ``ablations`` already drives) and score users against the loaded
    recommender. A user who just runs ``morel eval rank`` with no
    config gets the default Config's path executed end-to-end.
    """
    ...
    rank_cmd.add_argument("--items", type=int, default=50)
    rank_cmd.add_argument("--users", type=int, default=20)
    rank_cmd.add_argument("--epochs", type=int, default=1)
    ...
    robustness_cmd.add_argument("--items", type=int, default=50)
    robustness_cmd.add_argument("--users", type=int, default=20)
    robustness_cmd.add_argument("--epochs", type=int, default=1)
    ...
    if args.sub == "robustness":
        from morel.app import Robust as RobustApp

        config = load_cfg(resolve_cfg(argv))
        dir = Path("runs") / "robustness"
        result = RobustApp(
            config=config,
            dir=dir,
            items=args.items,
        ...
        for idx, ratio in enumerate(result["ratios"]):
            for k in sorted(config.eval.ks):
                recall_key = f"recall@{k}"
                if recall_key in result["metrics"]:
                    values = result["metrics"][recall_key]
                    if idx < len(values):
                        print(f"mask_ratio={ratio:.2f} {recall_key}={values[idx]:.4f}")
        return 0
    return 2
...
def fidelity(argv: list[str]) -> int:
    ...
    args = parser.parse_args(argv)
    from morel.core.fidelity import render, show
