...
class Updater:
    ...
    def undo(self, steps: int = 1) -> int:
        ...
        The ``load_state_dict`` call that restores the snapshot runs while
        the write lock is still held so a concurrent ``apply`` cannot
        observe the decremented version paired with the pre-load state.
        """
