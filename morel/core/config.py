...
class Config:
    ...
    def __post_init__(self) -> None:
        """Validate the freshly built config on every construction path.

        ``Config.parse`` already calls ``self.validate()`` so YAML/JSON
        loads keep the same guarantees, but every other construction path
        (``Config()``, ``Config.defaults()``, ``Config.env()``, in-process
        overrides in tests, builders in the CLI handlers, etc.) used to
        skip the validation pass and silently produce invalid state.
        Routing that through ``__post_init__`` makes the validation
        guaranteed regardless of how the config was built.
        """
        self.validate()
